...
int rplidar::start()
{
    ...
    ktorePosielam=-1;
    stopMeasurement=0;
    std::function<void(void)> f =std::bind(&laserVlakno, (void *)this);
    threadHandle=std::move(std::thread(f));
    //threadID=threadHandle.get_id();
    ...
    while(stopMeasurement==0)
    {
        ...
        if(oldP==5)
        {
            ...
            if((dataR[0]&0x01)&&( (dataR[0]&0x02) ==0))
            {
                ...
                if (sendto(s, &localMeranie[kdeJeCele].Data, sizeof(LaserData)*localMeranie[kdeJeCele].numberOfScans, 0, (struct sockaddr*) &si_posli, slen) == -1)
                {
