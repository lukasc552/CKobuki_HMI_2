...
long double CKobuki::getTickToMeter()
{
    return tickToMeter;
}

long double CKobuki::getB()
{
    return b;
}
...
std::vector<unsigned char> CKobuki::setTranslationSpeed(int mmpersec)
{
    ...
    if(mmpersec >= 0){ // Zistenie ci koleso ide dopredu alebo dozadu
        leftForward = true;
        rightForward = true;
    }else{
        leftForward = false;
...
std::vector<unsigned char> CKobuki::setRotationSpeed(double radpersec)
{
    ...
    if(radpersec > 0){// Zistenie ci koleso ide dopredu alebo dozadu
        leftForward = false;
        rightForward = true;
    }else if(radpersec < 0){
        leftForward = true;
